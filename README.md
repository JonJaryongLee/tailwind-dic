...
이 문서를 읽으려면 적어도 CSS 기초는 알아야 한다. 읽어서 잘 모르겠다면 CSS 의 해당 부분을 학습하고 다시 오자.
...
ex) `leading-tight` : 타이트한 줄간격

종류

`tight` < `snug` < `normal` (default) <  `relaxed` < `loose`
...
투명하게 하려면 `transparent` 사용

ex) `border-l-transparent`
